...
public class FieldView implements BaseView {
    ...
    private in.dragonbra.view.TileView[][] tileViews;
    private in.dragonbra.view.TileView[] fillerTileViews;

    //Booleans for animation, input disabling etc.
    private boolean touchDown = false;
    private boolean animating = false;

    private in.dragonbra.view.TileView selectedTile;
    private in.dragonbra.model.enumeration.SlideDirection slideDirection;
    ...
    private in.dragonbra.view.OrientationIndicator orientationIndicator;
    ...
    private void init() {
        ...
        tileViews = new in.dragonbra.view.TileView[in.dragonbra.Consts.FIELD_ROWS][in.dragonbra.Consts.FIELD_COLUMNS];
        fillerTileViews = new in.dragonbra.view.TileView[in.dragonbra.Consts.FIELD_COLUMNS];

        for (int i = 0; i < in.dragonbra.Consts.FIELD_ROWS; i++) {
            for (int j = 0; j < in.dragonbra.Consts.FIELD_COLUMNS; j++) {
                tileViews[i][j] = new in.dragonbra.view.TileView();
            }
            fillerTileViews[i] = new in.dragonbra.view.TileView();
        }

        orientationIndicator = new in.dragonbra.view.OrientationIndicator();
    ...
    public void render(ShapeRenderer shapeRenderer) {
        ...
        for (int i = 0; i < in.dragonbra.Consts.FIELD_ROWS; i++) {
            for (int j = 0; j < in.dragonbra.Consts.FIELD_COLUMNS; j++) {
                in.dragonbra.view.TileView view = tileViews[i][j];
        ...
        switch (slideDirection) {
            ...
            case ANTI_DIAGONAL:
                if (rowIndex < 4) {
                    ...
                    rightFillerPosY = offsetY + in.dragonbra.Consts.FIELD_ROWS * tileHeight + slideVector.y;
                } else {
            ...
            default:
                if (rowIndex < 4) {
                    ...
                    rightFillerPosY = offsetY + fillerIndex * tileHeight + slideVector.y;
                    leftFillerPosY = offsetY + in.dragonbra.Consts.FIELD_ROWS * tileHeight + slideVector.y;
                }
                break;
        }

        in.dragonbra.view.TileView filler = fillerTileViews[fillerIndex];
     ...
     */
    private void renderDuplicates(in.dragonbra.view.TileView original, ShapeRenderer shapeRenderer) {
    ...
    private void renderBorders(ShapeRenderer shapeRenderer) {
        ...
        in.dragonbra.shape.Rectangle rectangle = new in.dragonbra.shape.Rectangle();
        ...
        in.dragonbra.shape.Triangle triangle = new in.dragonbra.shape.Triangle();
    ...
    public void setDimensions(int width, int height) {
        ...
        tileWidth = (float) (screenWidth / Math.ceil(in.dragonbra.Consts.FIELD_COLUMNS / 2.0));
        ...
        offsetY = (screenHeight - (in.dragonbra.Consts.FIELD_ROWS - 1) * tileHeight) / 2.0f;
    ...
    public void setDrag(in.dragonbra.model.enumeration.SlideDirection direction, float dst) {
    ...
    public void animateNoMatchSlide() {
        ...
        if (slideVector.len() > 0) {
            animating = true;
            Tween.to(slideVector, in.dragonbra.animation.Vector2Accessor.POS_XY, in.dragonbra.Consts.SLIDE_DURATION)
                    .target(0, 0)
                    .ease(Cubic.OUT)
    ...
    public void animateFinishShift() {
        ...
        if (slideVector.len() > 0) {
            ...
            float time = (float) Math.sqrt(slideVector.len() * tileWidth / 2.0f)
                    * in.dragonbra.Consts.MAGIC_SLIDE_CONSTANT2;
            Tween.to(slideVector, in.dragonbra.animation.Vector2Accessor.POS_XY, time)
                    .target(0, 0)
                    .ease(Cubic.OUT)
                    .setCallback(new TweenCallback() {
                        @Override
    ...
    public in.dragonbra.model.enumeration.SlideDirection getSlideDirection() {
    ...
    public in.dragonbra.view.TileView getSelectedTile() {
        return selectedTile;
    }
    ...
    public in.dragonbra.view.TileView[][] getTileViews() {
        return tileViews;
    }

    public in.dragonbra.view.TileView[] getFillerTileViews() {
