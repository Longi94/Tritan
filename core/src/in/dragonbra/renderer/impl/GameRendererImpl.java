package in.dragonbra.renderer.impl;
...
import in.dragonbra.animation.TileViewAccessor;
import in.dragonbra.animation.Vector2Accessor;
import in.dragonbra.input.TritanInputProcessor;
...
public class GameRendererImpl implements GameRenderer, Disposable, FieldView.OnAnimationListener,
        TritanInputProcessor.TritanInputListener, FieldView.OnSlideEndListener {

    private static final String TAG = GameRendererImpl.class.getSimpleName();

    private Logger logger;

    private in.dragonbra.controller.GameController controller;
    ...
    public void notifyNewTilesGenerated() {
        ...
        for (int i = 0; i < in.dragonbra.Consts.FIELD_ROWS; i++) {
            for (int j = 0; j < in.dragonbra.Consts.FIELD_COLUMNS; j++) {
                ...
                in.dragonbra.model.Tile tile = field.getTiles()[i][j];
                view.setTile(tile);
                ...
                if (offset > 0) {
                    ...
                    float time = (float) Math.sqrt(offset * fieldView.getTileWidth() / 2.0f)
                            * in.dragonbra.Consts.MAGIC_SLIDE_CONSTANT;
    ...
    public void setController(in.dragonbra.controller.GameController controller) {
